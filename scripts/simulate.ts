...
async function main() {
    ...
    */
    ...
    console.log(`[Party A]: aDai Balance of Party A: ${hre.ethers.utils.formatEther(partyAInitialbalanceA)} aDAI`);
    console.log(`[Party A]: aWBTC Balance of Party A: ${hre.ethers.utils.formatEther(partyAInitialbalanceB)} aWBTC`);
    console.log(`[Party B]: aDai Balance of Party B: ${hre.ethers.utils.formatEther(partyBInitialbalanceA)} aDAI`);
    console.log(`[Party B]: aWBTC Balance of Party B: ${hre.ethers.utils.formatEther(partyBInitialbalanceB)} aWBTC`);

    // Party A approves AALPS contract to spend tokens
    await tokenA.connect(partyA).approve(aalps.address, hre.ethers.utils.parseEther("50"));
    console.log(`[Party A]: Approved tokens`);

    // Party B approves AALPS contract to spend tokens
    await tokenB.connect(partyB).approve(aalps.address, hre.ethers.utils.parseEther("0.001"));
    ...
    const createAALPSProposal = await aalps.connect(partyA).createAALPSProposal(
        partyB.address,
        secretHash,
        deadline,
        tokenA.address,
    ...
    const proposalCreatedEvent = txReceipt.events.find(event => event.event === 'AALPSERC20Lockup');
    ...
    const createAALPSProposalB = await aalps.connect(partyB).createAALPSProposal(
        partyA.address,
        secretHash,
        deadlineB,
        tokenB.address,
    ...
    const proposalCreatedEventB = txReceiptB.events.find(event => event.event === 'AALPSERC20Lockup');
    ...
    const AALPSInstance = await aalps.connect(partyB).getAALPSInstance(proposalIdB);
    const publiclyAvailableSecret = AALPSInstance["key"];

    console.log(`[Party B]: The publicly available secret is ${publiclyAvailableSecret}`);

    // Party B now knows the secret and can withdraw Party A’s funds
    await aalps.connect(partyB).withdrawTokens(proposalId, publiclyAvailableSecret);
    ...
    console.log(`[Party A]: aDai Balance of Party A: ${hre.ethers.utils.formatEther(partyAbalanceA)} aDAI`);
    console.log(`[Party A]: aWBTC Balance of Party A: ${hre.ethers.utils.formatEther(partyAbalanceB)} aWBTC`);
    console.log(`[Party B]: aDai Balance of Party B: ${hre.ethers.utils.formatEther(partyBbalanceA)} aDAI`);
    console.log(`[Party B]: aWBTC Balance of Party B: ${hre.ethers.utils.formatEther(partyBbalanceB)} aWBTC`);
}
